...
- V6 adds exactly that tie-breaker: when live-repair feature margins are tiny, choose the live repair with stronger masked-before trainability. On the current `13`-seed boundary synthesis, V6 matches the best evaluated candidate on `13/13`, is positive vs magnitude on `10/13`, and averages `+3.53` points over magnitude. Seed 313 is a fresh V6 prospective guardrail run: V6 keeps magnitude, and magnitude is the best evaluated candidate.
