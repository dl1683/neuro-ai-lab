...
- Across all eight completed strong TinyViT seeds, the V4 projection is positive vs magnitude on `6/8`, matches the best evaluated candidate on `7/8`, and averages `+3.20` points over magnitude. The fresh seed-306 run falsifies the perfect projection: V4 selected attention+MLP repair and beat magnitude by `+0.59` points, but SynFlow was better at `+2.30` points over magnitude.
- V4's first fresh strong seed is feature-dominant and positive; seed 306 is the more important correction. Feature alignment and masked pre-finetune accuracy are not sufficient when low-alignment SynFlow still recovers better. The next selector needs a SynFlow recovery-prior term.
