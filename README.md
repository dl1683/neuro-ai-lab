...
- Across all eight completed strong TinyViT seeds, the V4 projection is positive vs magnitude on `6/8`, matches the best evaluated candidate on `7/8`, and averages `+3.20` points over magnitude. The fresh seed-306 run falsifies the perfect V4 projection: V4 selected attention+MLP repair and beat magnitude by `+0.59` points, but SynFlow was better at `+2.30` points over magnitude.
- V5 adds a SynFlow masked-recovery prior: if SynFlow's masked-before accuracy is at least magnitude and close to the selected repair, prefer SynFlow. Projected over the eight completed strong TinyViT seeds, V5 matches the best evaluated candidate on `8/8`, averages `+3.41` points over magnitude, and leaves zero gap to the evaluated oracle. This is still a projection and needs a fresh prospective V5 run.
- The first fresh V5 prospective seed selected SynFlow and beat magnitude by `+2.93` points while zero-dead liveness masks stayed near magnitude. This supports the feature/SynFlow branch but does not yet validate the ambiguous branch prospectively.
