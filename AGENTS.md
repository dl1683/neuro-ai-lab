...
- **Blackboard thinking during waits (explore/exploit balance).** Whenever the loop is waiting — on a training run, a review, a gate result — that idle time is used for exploration, by Claude and Codex alike. Run genuine question-loop sessions that take nothing as given: requestion the assumptions of the current path; re-read accumulated data (including anomalies and nuisances — model-empty responses, protocol haircuts, padding sensitivity are DATA) in alternative ways; import solutions from other fields (psychometrics, metrology, optimal stopping, anytime algorithms, biology) that solved isomorphic problems; and work backwards from "what question are we not asking?". Exploitation follows the current path; exploration finds new ones and brings the survivors back through the steering dialogue. A wait that produced no recorded thought is wasted compute. Record exploration outputs in scratch, promote surviving insights via steering, and never let them silently redirect frozen protocols — new paths enter through preregistration like everything else.
...
- **Compute-cap policy.** Cap any single experiment run at approximately 2.5 GPU-hours and prefer many small hypothesis tests over scaling a single run. Exceeding the cap requires explicit owner approval. Long programs are deferred by policy, not scientific closure.
