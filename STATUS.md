...
| 07 Best-of-N safe selection | PREFLIGHT IMPLEMENTATION-DISCREPANCY STOP / CANONICAL NOT RUN | The retained 2-problem x 16-candidate calibration smoke remains immutable. The published verifier golden replay failed the unchanged 0.002 BF16 tolerance under both predeclared documented-compatible isolated stacks: Transformers 4.47.1 and 4.48.0 each produced maximum absolute error 0.0361328125 on the exact frozen input/tokenizer/checkpoint. The two-attempt boundary is exhausted; no further generation, scoring, verifier swap, or threshold change is authorized without round-2 steering and fresh registration where required. | `experiments/07_safe_selection/PREREGISTRATION.md`, `experiments/07_safe_selection/exp_f1_bon_safe_selection.py` |
