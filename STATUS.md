...
| 06 UESD — semantic ratchet | BASE-A TASK-BAND TERMINAL VOID / BASE-B TASK-BAND PASS / MECHANICS INDEPENDENT | Base-A remains terminal **VOID** after its GSM8K below-band result and inapplicable SVAMP parser-repair branch. The single disjoint base-B GSM8K successor gate **PASS**ed with 143/256 correct numeric, 104/256 valid extracted incorrect, 0/256 model-empty, and 9/256 parser-recognition failures. This is band placement only; the mechanism remains untested. | `experiments/06_uesd/results/exp_e1_task_band_base_b.json`, prior base-A artifacts, `experiments/06_uesd/PREREGISTRATION.md` |
...
The 30M shortcut-resistant synthetic-deduction mechanics pilot remains formally **INDEPENDENT** of base-A, base-B, GSM8K, SVAMP, and the real-task task-band gate. Its state is unchanged: implemented, with its own independent pre-training code review still pending. Base-B has satisfied the separate task-band prerequisite, but the full program remains blocked until the mechanics pilot has an admissible outcome; before full training, a separate pre-data amendment must also freeze a new untouched cross-family transfer set, followed by launch review. The revised pre-data budget tied to base-B selection is approximately **90–150 GPU-hours**, with token budgets and the 5% trainable-overhead ceiling unchanged.
