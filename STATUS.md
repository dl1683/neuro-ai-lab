...
Audited snapshot through the terminal base-A/SVAMP adjudication and the single canonical base-B successor-gate `PASS`. The immutable base-A initial-miss artifact remains unchanged. The base-B artifact is immutable band-placement evidence only.
...
The 30M shortcut-resistant synthetic-deduction mechanics pilot remains formally **INDEPENDENT** of base-A, base-B, GSM8K, SVAMP, and the real-task task-band gate. It is implemented and its independent pre-training review is clean under attestation `INDEPENDENT_PRETRAINING_REVIEW_CLEAN fba181cd30f6465ce4149c10fd08eab459e1b0bf`; the reviewed E2 launch conditions are met. No E2 run has occurred. Base-B has satisfied the separate task-band prerequisite.

The 0.5B program is **DEFERRED — compute policy (2026-08-10)**. Its preregistration and amendments remain valid prospective registrations; deferment is policy, not scientific closure and not a KILL or VOID. The approximately **90–150 GPU-hour** program exceeds the standing approximately 2.5 GPU-hour single-run cap and cannot launch without explicit owner approval. If that approval is later given, the scientific gates still require an admissible mechanics outcome, a pre-data amendment freezing a new untouched cross-family transfer set, and launch review; the token budgets and 5% trainable-overhead ceiling remain unchanged.
...
Current runner states (2026-08-10): E2 mechanics pilot IMPLEMENTED (four arms, self-tested) — independent pre-training review clean under `INDEPENDENT_PRETRAINING_REVIEW_CLEAN fba181cd30f6465ce4149c10fd08eab459e1b0bf`; reviewed launch conditions are met. Base-B successor gate COMPLETE / PASS after the single reviewed canonical attempt. No E2 run has occurred. The 0.5B program is **DEFERRED — compute policy (2026-08-10)** while all prospective registrations remain valid.

The fixed-point convergence arc is closed. D40 stands at 15/16; rerunning the crashed run (seed 512, λ_sc=3.0) is publication-completeness work, not a blocker to the negative conclusion. The separately preregistered semantic-ratchet mechanism and full-program experiment have not been run and are not evidence. The base-A GSM8K task-band gate completed as ABORT-AND-SWAP, and its SVAMP fallback is terminal VOID under the parser-repair-scope amendment. The disjoint base-B successor gate completed as PASS; this satisfies only the band-placement prerequisite. The mechanics pilot is independent and has met its reviewed launch conditions; full training remains policy-deferred and, if explicitly approved, still requires mechanics admissibility plus the replacement-transfer-set amendment and launch review. `exp_d14_scaling_laws.py` and `exp_d26_criticality_recovery.py` are reviewed-but-unexecuted designs with no result JSON; do not cite them as empirical evidence.
