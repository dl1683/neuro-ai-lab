...
| 07 Best-of-N safe selection | INTERRUPTION-BOUNDED PERFORMANCE FORK IMPLEMENTED / READ-ONLY REVIEW AND HASH BINDING BLOCKED / NO RETAINED DATA / NO SCIENTIFIC OUTCOME | The Option-A successor remains closed at `PREFLIGHT_STOP_COMPUTE_CAP`, and the first one-shot performance fork remains terminal `CLOSED_INTERRUPTED_BRANCH_1_GOVERNS`. The separate Round-40 fork now implements a hash-chained write-once ledger, atomic timing-pair boundaries, terminal mid-pair closure, exact-equivalence plus independent 1.67x and 8,100s gates, and no automatic cap exception. CPU tests pass, but both preregistration hash slots remain unfilled. A separate clean read-only review and exact hash binding are required before GPU work. | `experiments/07_safe_selection/PREREGISTRATION.md`, `experiments/07_safe_selection/exp_f1_bon_safe_selection.py`, `experiments/07_safe_selection/results/exp_bon_safe_selection_performance_fork.json` |
...
existing runner. CPU-only validation passed 11/11 stopping property cases, 8/8
lifecycle checks, and 4/4 independent arithmetic-gate checks; the lifecycle
...
unfilled. Clean read-only pre-launch review and exact runner/review hash binding
are still required in a separate session.
