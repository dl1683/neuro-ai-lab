...
Audited snapshot through the terminal base-A/SVAMP adjudication and the pre-data base-B successor-gate amendments. The immutable initial-miss artifact remains unchanged. No base-B response, smoke evaluation, or canonical evaluation exists.
...
| 06 UESD — semantic ratchet | BASE-A TASK-BAND TERMINAL VOID / BASE-B GATE PREREGISTERED / MECHANICS INDEPENDENT | The base-A GSM8K gate remains **ABORT-AND-SWAP** at 18/256 correct. The base-A/SVAMP fallback is terminal **VOID**: 13/256 model-empty extraction failures (5.078125%) exceed the 5.000% ceiling, and no recoverable numeric content exists for a permitted parser repair. The disjoint base-B GSM8K successor gate is **PREREGISTERED / NOT YET RUN**. | `experiments/06_uesd/results/exp_e1_task_band.json`, `experiments/06_uesd/results/exp_e1_task_band_svamp_initial_parser_miss.json`, `experiments/06_uesd/PREREGISTRATION.md` |
...
One successor gate is **PREREGISTERED / NOT YET RUN** for frozen `base-B`: exactly 256 revision-pinned GSM8K official-test examples selected by the precommitted deterministic construction, disjoint from the base-A cohort, with no fallback. No base-B response, smoke evaluation, or canonical evaluation existed before the amendment. Its runner requires independent pre-launch review before any generation.

The 30M shortcut-resistant synthetic-deduction mechanics pilot is now formally **INDEPENDENT** of base-A, base-B, GSM8K, SVAMP, and the real-task task-band gate. It may proceed only after its own independent pre-training code review. The full program remains blocked unless both the mechanics pilot has an admissible outcome and the base-B gate passes; before full training, a separate pre-data amendment must also freeze a new untouched cross-family transfer set. The revised pre-data budget tied to base-B selection is approximately **90–150 GPU-hours**, with token budgets and the 5% trainable-overhead ceiling unchanged.
