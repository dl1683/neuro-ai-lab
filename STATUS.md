...
Audited snapshot through the terminal base-A/SVAMP adjudication, the single canonical base-B successor-gate `PASS`, the single E2 mechanics-pilot attempt with a controlling post-evidence `VOID`, and the completed owner-authorized operational repair of E2-DIAG Stage 0. The repaired diagnostic adjudicates the registered Stage-0 `STOP` and routes `KILL_FROM_SCRATCH_LINE`; its instrumentation found no broken downstream gradient path. The base-B artifact is immutable band-placement evidence only; E2 and E2-DIAG are diagnostic/design-gate evidence only.
...
| 06 UESD — semantic ratchet | BASE-A TASK-BAND TERMINAL VOID / BASE-B TASK-BAND PASS / E2 CONTROLLING VOID / E2-DIAG STAGE-0 STOP | Base-B satisfied the separate band-placement prerequisite. E2 remains controlling `VOID` at 0/200 confidence-matched pairs per seed. The repaired E2-DIAG Stage 0 stayed at 32/128 through 3,000 updates with healthy downstream gradients and 3,000/3,000 clipped updates; the frozen branch table assigns `KILL_FROM_SCRATCH_LINE`. No later diagnostic stage or second repair is authorized. Official tests remain uninspected and the 0.5B launch remains blocked. | `experiments/06_uesd/results/exp_e2_diag_stage0_instrumented.json`, `experiments/06_uesd/results/exp_e2_diag.json`, `experiments/06_uesd/results/exp_e2_latch_mechanics.json`, `experiments/06_uesd/results/exp_e1_task_band_base_b.json` |
...
E2-DIAG Stage 0 is **COMPLETE / REGISTERED `STOP` / DIAGNOSTIC ONLY** after the single owner-authorized operational repair. The repaired invocation completed 3,000 updates over 2,786,051 non-padding input-plus-answer tokens in 98.873 GPU-seconds, with peak allocated/reserved VRAM of 637,935,104/696,254,464 bytes. T=4 training accuracy was exactly 32/128 at initialization and every 100-update checkpoint, below the registered 122/128 gate. Full-set mean CE decreased from 1.522216796875 to a minimum 1.38671875 at update 2,700—still above `ln(4)`—and ended at 1.388671875. Every post-update checkpoint predicted a single answer class for all 128 examples; the class changed in 21/30 intervals, yielding 2,682 example-level flips without changing balanced-label accuracy.

The gradient instrumentation found no severed model path. Every tensor in the encoder, controller, plan-slot, prefix-projector, answer-decoder, and readout groups had a nonzero gradient at all 30 samples. The readout-head weight delta reached 1.925192 L2, so its weights updated. All 3,000/3,000 updates exceeded the clip norm. This supports an optimization-saturation post-mortem rather than the suspected `detach`/masking/readout wiring bug, but the telemetry is informational: the frozen branch table controls and assigns **`KILL_FROM_SCRATCH_LINE`**. It does not authorize Stage 1, the Stage-2 optimizer matrix, a model fix, or a successor launch. The repaired immutable artifact is `experiments/06_uesd/results/exp_e2_diag_stage0_instrumented.json` (canonical-LF SHA-256 `0ad4fc5fafe343b37944b462972d32ff201749280a7bdbb506fa3b62536620d7`). The first operational-VOID artifact remains immutable at `experiments/06_uesd/results/exp_e2_diag.json` (canonical-LF SHA-256 `41c035dd79fe42e9edb891dcc02664077c7d491cd7a2857fe7cc9ae16dc5ab37`). No official-test, selector, critic, or latch path was accessed; the original E2 controlling `VOID`, blocked 0.5B launch, absence of an automatic Direction 2 route, and line-07 priority remain unchanged.
...
Current runner states (2026-08-10): E2 mechanics pilot COMPLETE / raw artifact `FAIL` / controlling post-evidence `VOID` after the single independently reviewed canonical attempt. E2-DIAG Stage 0 COMPLETE / `STOP` / `KILL_FROM_SCRATCH_LINE` after the single owner-authorized repair; no later diagnostic stage, second repair, resume, or model fix is authorized. Official tests were not inspected. Base-B successor gate remains COMPLETE / PASS after its single reviewed canonical attempt. The current 0.5B semantic-ratchet launch is blocked; no full-experiment route is automatically unlocked. The immutable E2 and both E2-DIAG artifacts must not be edited, regenerated, or relocated.
