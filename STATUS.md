...
Audited snapshot through the terminal base-A/SVAMP adjudication, the single canonical base-B successor-gate `PASS`, the single E2 mechanics-pilot attempt with a controlling post-evidence `VOID`, and E2-DIAG Stage 0 with a terminal operational `VOID_NO_ROUTE`. The E2-DIAG process completed the registered 3,000-update training endpoint but crashed during post-endpoint result construction, so the observed 32/128 training count is execution evidence rather than an adjudicated Stage-0 `STOP`. The base-B artifact is immutable band-placement evidence only; E2 and E2-DIAG are diagnostic/design-gate evidence only.
...
| 06 UESD — semantic ratchet | BASE-A TASK-BAND TERMINAL VOID / BASE-B TASK-BAND PASS / E2 CONTROLLING VOID / E2-DIAG STAGE-0 OPERATIONAL VOID | Base-B satisfied the separate band-placement prerequisite. E2 remains controlling `VOID` at 0/200 confidence-matched pairs per seed. E2-DIAG reached the 3,000-update endpoint at an observed 32/128 training count, then failed during result construction; frozen operational precedence assigns `VOID_NO_ROUTE`, no scientific branch, and no repeat. Official tests were not inspected. The 0.5B launch remains blocked. | `experiments/06_uesd/results/exp_e2_diag.json`, `experiments/06_uesd/results/exp_e2_latch_mechanics.json`, `experiments/06_uesd/results/exp_e1_task_band_base_b.json`, prior base-A artifacts, `experiments/06_uesd/PREREGISTRATION.md` |
...
E2-DIAG Stage 0 is now **TERMINAL OPERATIONAL `VOID_NO_ROUTE` / DIAGNOSTIC ONLY**. The single invocation completed 3,000 updates over 2,786,051 non-padding input-plus-answer tokens and printed 32/128 correct (25% training accuracy) at every registered 100-update observation. Stdout-rounded T=4 training CE ranged from 1.384766 to 1.521240 and ended at 1.388611. After the endpoint printed, result construction crashed on a quantile-tensor dtype mismatch before the gradient distribution, clipped-update numerator, recovery checkpoint, exact GPU wall time, or peak VRAM could be published. Under the frozen integrity/operational precedence, the 32/128 observation is non-adjudicating execution evidence: it is not a Stage-0 `STOP`, does not assign `KILL_FROM_SCRATCH_LINE`, does not advance to Stage 1, and cannot select any optimizer or successor branch. No rerun or resume is permitted. The immutable artifact is `experiments/06_uesd/results/exp_e2_diag.json` (canonical-LF SHA-256 `41c035dd79fe42e9edb891dcc02664077c7d491cd7a2857fe7cc9ae16dc5ab37`). Official-test and selector paths were not accessed; the original E2 controlling `VOID`, blocked 0.5B launch, absence of an automatic Direction 2 route, and line-07 priority remain unchanged.
...
Current runner states (2026-08-10): E2 mechanics pilot COMPLETE / raw artifact `FAIL` / controlling post-evidence `VOID` after the single independently reviewed canonical attempt. E2-DIAG Stage 0 is terminal operational `VOID_NO_ROUTE` after its single invocation; no later diagnostic stage or repeat is authorized. Official tests were not inspected. Base-B successor gate remains COMPLETE / PASS after its single reviewed canonical attempt. The current 0.5B semantic-ratchet launch is blocked; no full-experiment route is automatically unlocked. The immutable E2 and E2-DIAG artifacts must not be edited, regenerated, or relocated; local recovery provenance does not authorize a rerun.
