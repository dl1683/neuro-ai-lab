...
| 07 Best-of-N safe selection | FRESHLY PREREGISTERED OPTION-A SUCCESSOR / ORIGINAL ATTEMPT TERMINAL PREFLIGHT STOP / NOT YET RUN | The original attempt remains closed at `PREFLIGHT_STOP_IMPLEMENTATION_DISCREPANCY`, and its retained 2-problem x 16-candidate smoke remains immutable historical design data. A fresh successor is now prospectively registered around the exact hash-bound `verifier-V / maintained-eager-BF16-v1` scorer, entirely new cohorts drawn from the 6,700 previously unallocated rows, exact three-load/two-process determinism, and an outcome-blind calibration rescore gate. Implementation and pre-review checks are authorized; retained generation remains blocked on filled hash slots and holistic review. | `experiments/07_safe_selection/PREREGISTRATION.md`, `experiments/07_safe_selection/exp_f1_bon_safe_selection.py` |
