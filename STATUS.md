...
| 07 Best-of-N safe selection | INTERRUPTION-BOUNDED PERFORMANCE FORK HASH-BOUND / GPU PROBE AUTHORIZED BUT NOT RUN / NO RETAINED DATA / NO SCIENTIFIC OUTCOME | The Option-A successor remains closed at `PREFLIGHT_STOP_COMPUTE_CAP`, and the first one-shot performance fork remains terminal `CLOSED_INTERRUPTED_BRANCH_1_GOVERNS`. The separate Round-40 fork implements an anchored hash-chained write-once ledger, resumable canonical-null-head promotion, atomic timing-pair boundaries, durable-start-bound orphan adjudication, suffix-truncation and missing-start closure, measured post-load snapshot revalidation, exact-equivalence plus independent 1.67x and 8,100s gates, and no automatic cap exception. CPU tests pass; delta review round 4 is CLEAN and both preregistration hashes are bound. The outcome-blind GPU probe is authorized. | `experiments/07_safe_selection/PREREGISTRATION.md`, `experiments/07_safe_selection/exp_f1_bon_safe_selection.py`, `experiments/07_safe_selection/results/codex_interruption_bounded_fork_prelaunch_review.md` |
...
broader runner self-test also passed. Delta review round 4 returned CLEAN. The
exact runner and persisted review SHA-256 values are now bound in the
preregistration, authorizing the outcome-blind GPU probe. Terminal fork reports
state the durability boundary: ordinary
process-kill windows are protected, but power-loss durability is not established
because Windows directory metadata fsync is unavailable.
