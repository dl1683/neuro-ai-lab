...
- **Claim boundary:** This is band-placement evidence for frozen `base-B` only. It is not a capability claim and supplies no evidence for the semantic-ratchet mechanism. The mechanics pilot remains implemented and independent; its clean independent pre-training review means E2 launch conditions are met, but no E2 run has occurred. The 0.5B program is deferred under the 2026-08-10 compute policy while its registrations remain valid; any later approved launch still requires an admissible mechanics outcome, a pre-data amendment freezing an untouched replacement cross-family transfer set, and launch review.
