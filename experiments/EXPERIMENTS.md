...
- **Cap denominator:** Batch 8 was the faster configuration at `g_b=0.9452875406s/response`; its full 768-problem/12,288-response projection was 12,731.6355s and the largest under-cap bank was 486 problems. Batch 16 projected 36,205.6470s and at most 171 problems. Both maxima are below the registered 512-problem minimum, so no cap-resolution PASS was legal.
- **Access boundary:** The probes executed 128/128 throwaway candidate generations across both batch sizes and duplicate executions; 0/128 entered a retained bank. Retained responses, successor candidate scores, calibration/test outcome accesses, viability metrics, rescore records, policy evaluations, bootstrap replicates, and permutations were all 0. No six-policy accuracy or switch result exists.
- **Compute and power:** Registered probe generation plus model-load wall time was 58.4025s for batch 8 and 130.8621s for batch 16 (189.2646s total). Sampled probe energy was 3.97725 Wh; peak allocated/reserved VRAM was 5,096,533,504/6,473,908,224 bytes. Launch was on Windows High Performance with the GPU at P0, 26.96 W idle, 0 MiB allocated, 0% utilization, and 53°C; no power-scheme transition was observed.
- **Evidence:** `experiments/07_safe_selection/results/exp_bon_safe_selection_cap_preflight.json` (canonical-LF SHA-256 `8f7d3be695f5503bd5a0a87a09651f52ab38a0adb4a04483d9a4be842d8da0c2`). This is an operational pre-retained-data stop, not CONFIRM, KILL, or VOID; the registered safe-selection law remains untested.

- **Implementation event (historical checkpoint):** The pseudonymous maintained-eager BF16 scorer, three-load/two-process determinism gate, fresh-cohort construction, deterministic schedules, preserved calibration headroom gate, and pre-test outcome-blind 64-record rescore gate were implemented. At that checkpoint every retained mode remained fail-closed pending holistic review.
...
- **Access boundary at that checkpoint:** No retained response was generated, no successor candidate was scored, and no calibration/test outcome, headroom metric, threshold, policy result, or scientific result existed. The separate successor work root did not open or modify the immutable historical smoke/database. The then-next action was holistic independent review; the controlling current boundary is the cap stop above.
...
### E3 Interface-Supervision Diagnostic (FROZEN_NOT_LAUNCHABLE / NOT RUN)
...
- **Blocking-review repair:** The 2026-08-11 pre-data amendment resolves I0 to a shared fixed 32-entity vocabulary with explicit skeleton-disjoint symbolic verification; attempts to counterbalance I2 entity/predicate/lexical/answer-position contingencies under both labels at every distance; replaces S2's fabricated premises with genuinely grounded registered-rule applications and exact validity-label feature balances; adds post-sync cell/suite checks and timed publication; records every affine-head optimizer boundary; and expands the CPU self-test across all miniature records plus adversarial route/time fixtures.
- **Implementation and bindings:** The amended runner/config, clarified registration protocol body, E2 generator and E3 reusable source, template inventory, proof-trace/candidate contract, tokenizer-containing substrate digest, three prompt serializations, and all six ordered split payloads are SHA-256-bound. The all-record CPU self-test checks 3,584 prompt rows, 512 process rows, and 7,168 candidates. No diagnostic result or metric exists.
- **Terminal re-review finding:** The bounded independent re-review found that I2 still leaks the label through the queried label-word's context occurrence count at every proof distance in both fact splits. A single-feature affine classifier using only that count reached 69.73% on miniature validation. Fixing it requires reconstructing matched-negative assignment and rebinding the fact datasets, which is a design-level reconstruction beyond the permitted correction cycle.
- **Controlling status:** **`FROZEN_NOT_LAUNCHABLE / NOT RUN / NO SCIENTIFIC CONCLUSION`** under the recalibration bottom-out rule. No cell, result artifact, or diagnostic scientific metric exists. The 69.73% value is a design-audit shortcut probe, not an interface or supervision result.
- **Reusable assets and open question:** The independently verified S2 grounded registered-rule distractors and shortcut checks, frozen-substrate/access boundaries, and exhaustive branch table remain reusable assets. The mechanics line's representation-versus-supervision routing question remains open, but only fresh steering plus registration may revisit it.
- **Claim and access boundary:** This freeze cannot rescue the E3 preflight, establish mechanics, authorize canonical E3/E3b or the 0.5B program, inspect official tests, modify line 07, or consume any line-07 cohort.
...
- **Instrument probe:** Both fixed throwaway initializations completed 25 updates. All 50 global pre-clip norms were finite and positive: min 7.6279, p05 9.0477, p25 11.3132, median 13.6205, p75 16.7096, p95 23.4852, max 28.9691. The frozen mapping selected `C=14`. No accuracy, prediction, class-collapse, validation loss, competence result, or forbidden cohort was computed or inspected.
- **Smoke curve:** Mean train CE over successive 50-update windows was `1.6670, 1.4671, 1.4739, 1.4593, 1.4359, 1.4383, 1.4302, 1.4250, 1.4344, 1.4062`. No intermediate validation accuracy or alternative horizon was inspected.
- **Controlling outcome:** **`PREFLIGHT_STOP / PRETRAINED_INTERFACE_COMPETENCE_SMOKE_MISS`**. The single update-500 evaluation at `T=4` scored 128/512 = 25.0% (95% Wilson interval 21.4448%–28.9275%), below the integer floor of 205/512. This is a pre-spend competence miss, not a canonical mechanics result.
- **Compute and isolation:** Overall wall time was 104.162s; the smoke occupied 72.015s against its 900s cap. Peak allocated/reserved VRAM was 3,453,325,824/3,873,439,744 bytes. Exactly 550 throwaway optimizer updates completed and zero canonical updates did. All weights, optimizers, RNG/dataloader state, and representation caches were discarded; no checkpoint was retained.
- **Access and route:** All seven fresh synthetic namespaces were pairwise disjoint. Zero line-07, GSM8K/SVAMP official-test, or E2-test rows were accessed, and the two line-07 worktree files were not written. No probe/smoke repeat, alternate horizon, interface change, dense supervision, extra update, or canonical launch is authorized. The only route is drafting a fresh `REGISTER_INTERFACE_SUPERVISION_DIAGNOSTIC` that separates representation-interface choice from answer-only versus verified dense process supervision. That route does not authorize a run or unblock the 0.5B program.
...
**Verdict:** registered Stage-0 `STOP → KILL_FROM_SCRATCH_LINE`. The one-seed diagnostic remained at 32/128 training examples correct through all 3,000 completed updates. Mean full-set CE moved from 1.522217 to a minimum of 1.386719, still above `ln(4)=1.386294`, and ended at 1.388672. Every post-update checkpoint predicted one class for all 128 balanced examples; that class changed in 21/30 checkpoint intervals, producing 2,682 example-level flips without an accuracy change. All tensors in every instrumented group had nonzero gradients at all 30 samples, the readout weights changed by 1.925192 L2, and 3,000/3,000 updates were clipped. The registered branch—not this post-hoc interpretation—controls: the current from-scratch 30M E2 substrate and training pipeline are killed, with no Stage 1, optimizer-matrix access, repair, or official-test inspection.
...
- **Artifact:** `experiments/06_uesd/results/exp_e2_latch_mechanics.json` (checkout-stable canonical-LF and runtime-byte SHA-256 `7842ca6f69ba3885fe7b03142b694e9c95950f195d31acd73f601d1e3f5a4075`). The artifact is immutable.
- **Protocol accounting:** Both seeds trained one common recurrent controller for exactly 500,000 non-padding input-plus-answer tokens and one parameter-matched encoder control for the same budget. Each seed supplied 491,520 selector-training states and 16,384 selector-calibration states, exceeding the preregistered 20,000 and 10,000 floors. Generator/split integrity, feature boundaries, two-seed preparation, token accounting, selector fitting, and confidence split/freeze contracts passed.
- **Calibration collapse:** For both seeds, no-latch accuracy was exactly 256/1,024 (25%) at every candidate horizon `T=1..15`; pooled accuracy was 512/2,048 (25%). The smaller-horizon tie-break froze `t*=1`, so observed calibration gain from `T=1` to `t*` was 0/1,024 in each seed and 0/2,048 pooled.
- **Pretest selector provenance:**

  | Metric | Seed 42 | Seed 31415 | Pooled | Frozen requirement |
  |---|---:|---:|---:|---:|
  | Critic AUROC | 0.5003444155 (25,183,159 concordant-pair units / 50,331,648 positive-negative pairs; 4,096 positive and 12,288 negative states) | 0.5021731555 (25,275,202.5 / 50,331,648; 4,096 positive and 12,288 negative states) | Not computed by the pretest adjudicator | >=0.75 |
  | Confidence-plus-schedule AUROC | 0.4999594887 (25,163,785 / 50,331,648 positive-negative pairs; 4,096 positive and 12,288 negative states) | 0.4996897976 (25,150,211 / 50,331,648; 4,096 positive and 12,288 negative states) | Not computed | Comparator |
  | Critic AUROC advantage | 0.0003849268 | 0.0024833580 | Not computed | >=0.05 |
  | Critic selection accuracy | 256/1,024 (25%) | 256/1,024 (25%) | Not computed | >=3 points above confidence |
  | Confidence selection accuracy | 256/1,024 (25%) | 256/1,024 (25%) | Not computed | Comparator |
  | Selection-accuracy advantage | 0 points | 0 points | Not computed | >=3 points |
  | Confidence-matched concordance | Undefined: 0 qualifying pairs | Undefined: 0 qualifying pairs | Not computed | >=0.70 |
  | On-policy critic AUROC | 0.5003444155 (25,183,159 / 50,331,648 positive-negative pairs; 4,096 positive and 12,288 negative states) | 0.5021731555 (25,275,202.5 / 50,331,648; 4,096 positive and 12,288 negative states) | Not computed | >=0.70 |

  The undefined matched-concordance rates have denominator zero; they are not observed zero AUROC. Every separately measurable provenance criterion nevertheless failed in both seeds.
- **Raw artifact token:** **`FAIL`**, reason `PRETEST_SELECTOR_PROVENANCE_GATE_MISSED`, failed seeds 2/2 (`42`, `31415`), invalid selector fits 0/2. The pretest branch stopped before official-test inspection.
- **Post-evidence adjudication:** **`VOID`**, reason `INSUFFICIENT_CONFIDENCE_MATCHED_PAIRS_PER_SEED`. Both seeds supplied 0 qualifying confidence-matched pairs against the preregistered minimum of 200 per seed. That floor must be satisfied before any `PROCEED`/`FAIL`, and the frozen precedence is `VOID > FAIL > PROCEED`. The runner applied this denominator gate only in the never-entered official-test branch, so its immutable raw `FAIL` is retained as execution evidence but is not the controlling scientific token.
- **Endpoint quantities:** Official test was not inspected. Therefore per-seed and pooled no-latch regression at the decision endpoint, critic-latch regression and reduction, gain retention, endpoint critic-minus-confidence accuracy, matched-pair AUROC, competence floors, encoder-control test accuracy, `O/F/H` headroom, transition hazards, paired bootstrap, and per-example records are all **not applicable**, not zero or failed.
- **Arm 4 informational summary:** Both seeds froze `delta=0.00` with `calibration_constraint_miss=true`; every calibration grid value had B=16 accuracy 256/1,024 and undefined gain retention (0/0 because `t*` gain was zero). Harmful-switch suppression and accuracy at B=16/B=32 on test are not applicable because the test grid was not evaluated. Arm 4 cannot alter either the immutable runner-emitted raw `FAIL` or the controlling post-evidence `VOID`.
- **Compute and power:** Artifact wall time was 1,181.121863s (19m41.12s), with peak CUDA memory 1,076,839,936 bytes allocated and 1,256,194,048 bytes reserved. Launch was `2026-08-10T19:08:07.0249142Z`; AC was online, battery was 99% and charging/high, the Windows High performance plan was active, and sampled GPU draw at +20s was 28.85 W. No battery transition, crash, stall, retry, or protocol deviation occurred.
- **Claim boundary and route:** This is mechanics design-gate diagnostics, not publishable mechanism evidence. It supports only that the fitted latent critic missed every separately measurable provenance threshold and showed no meaningful advantage over confidence-plus-schedule on the frozen pretest selector data; the zero-denominator matched population prevents an admissible `FAIL`. It does not show that a latch failed at H=16/H=32, that overthinking was absent, or that latent-state selection is impossible generally. The 0.5B semantic-ratchet program does not launch. Because the controlling token is `VOID`, no automatic Direction 2 route was unlocked. The separately registered E2-CERT follow-up cannot rescue, delay, or reinterpret E2 and is not authorized by this outcome.

**INTERPRETATION — E2/E2-DIAG post-mortem (non-adjudicating; source: 2026-08-10 steering convergence):**

The combined E2 and E2-DIAG evidence revises the leading explanation from generic substrate starvation alone to an **optimization-saturated from-scratch regime at every observed budget**. E2 supplied sparse four-way supervision over very few effective updates, while the longer Stage-0 memorization test clipped all 3,000/3,000 updates and decayed its learning rate without improving beyond 32/128. Healthy downstream gradients rule against a severed computation path, but do not show that the current clip/LR regime would fail under every possible budget or that changing optimization would necessarily recover learning. Those possibilities belong only to a materially different, freshly preregistered successor.

### E1 Base-B Successor Task-Band Gate (COMPLETE — PASS)
- **Config:** `experiments/06_uesd/exp_e1_task_band.py` (executed from commit `a43bd5c` with `--dataset gsm8k --successor-base-b`)
- **Artifact:** `experiments/06_uesd/results/exp_e1_task_band_base_b.json` (checkout-stable canonical-LF SHA-256 `9c57a10f3aa64c43fa34819255f4cf4e004cc5ab74afaa838c02c4a06a271de2`; original runtime-byte SHA-256 `2c78f7ad83e6481a6abbdf82146945da5592cef24b91ea0bb1f007a424a3f0ca`)
- **Purpose and protocol:** Execute the single preregistered successor gate for frozen `base-B` on exactly 256 revision-pinned GSM8K official-test examples. The deterministic cohort hash is `670705ea2936f75f0e90a4048d3f5b5ec3a63b42577c0d7a9df87253b77444ff`; all 256 indices are unique, overlap with the base-A cohort is 0/256, and 807 official-test examples remain unconsumed. The run used the fixed five-shot prompt, native chat serialization, greedy batch-1 decoding, 256-token cap, and initial frozen exact-rational parser. Leakage preflight passed with 0/256 overlaps, and repeat determinism passed on 2/2 checked examples.
- **Four-category accounting:** 143/256 `correct_numeric` (55.859375%); 104/256 `valid_extracted_incorrect` (40.625%); 0/256 `model_empty_non_answer` (0%); and 9/256 `parser_recognition_failure` (3.515625%). These mutually exclusive counts sum to 256/256. The usable-incorrect population is 104/256.
- **Generation diagnostics:** 40,042 tokens were generated. Mean response length was 156.41 tokens (median 151.5; nearest-rank p95 256), and 23/256 responses (8.984375%) reached the 256-token cap.
- **Compute:** 21,412.46s total wall time (5h 56m 52.46s), including 21,391.56s generation and 11.28s repeat verification. Artifact-recorded throughput was 1.87186 generated tok/s. Peak CUDA memory was 3,278,870,016 bytes allocated and 3,716,153,344 bytes reserved (12.78% / 14.49% of 25,650,855,936 device bytes).
- **Verdict:** **`PASS`** with reason `all_successor_task_band_thresholds_satisfied`: correct count is inside 26–217 and at least 40; usable incorrect count is at least 40; model-empty non-answers and parser-recognition failures are each at most 12/256; and the frozen provenance, leakage, determinism, and accounting checks pass.
- **State at Base-B landing:** This was band-placement evidence for frozen `base-B` only, not a capability claim or semantic-ratchet mechanism evidence. At that point, the independent mechanics pilot had not run and the 0.5B program was compute-deferred. The later E2 attempt and controlling post-evidence `VOID` are recorded in the newer entry above; this historical note does not override them.
- **Selection-prior retrospective:** The supplied approximately 19% projection implied about 49/256 correct; the observed 143/256 was approximately 2.9× higher. This was a failed selection prior — **a prior, not evidence** — and is not a capability baseline.
- **Operational deviations:** The run launched with the required ignored workspace cache and fully offline flags. Early telemetry showed active decoding at roughly 19.7–21.0 W. A transient higher-power interval began around 02:25:54 local (one sample at 113 W, then roughly 44–52 W) and returned to about 20 W by 02:35:47; after a monitoring gap, active draw was again roughly 44–88 W through completion. Throughput is not a frozen protocol parameter, so no scientific protocol changed. The 5h56m wall time exceeded the preregistered 1.3–3.5h estimate; the process was not stopped because GPU generation remained active and no genuine 15-minute stall was observed.
...
- **Base-B successor gate runner PREPARED** (`3077598` → `b42bc82`): disjoint-cohort construction (hash `670705ea…444ff` verified), four-category outcome taxonomy, batch-1 repeat-determinism, repaired-attempt immutability with five identity hashes and TOCTOU-atomic publication, complete terminal-VOID coverage. Hardened through four independent review rounds before the single canonical attempt recorded above.
...
### Semantic-Ratchet Direction (HISTORICAL PREREGISTRATION EVENT)
...
- **Evidence status at registration:** Zero semantic-ratchet mechanism-training runs, metrics, or result artifacts existed when this event was logged. The later E2 mechanics attempt, its immutable runner-emitted raw `FAIL`, and the controlling post-evidence `VOID` are recorded above; the full 0.5B program was not run. The preregistration remains the frozen protocol governing that outcome, not empirical evidence itself.
...
- **Status note:** Run 16 (seed 512, λ_sc=3.0) crashed in Phase B `loss.backward()` with `torch.AcceleratorError: CUDA error: an illegal memory access was encountered` (cudaErrorIllegalAddress; full traceback in local `results/_d40_stderr.log`, not committed). Checkpoints for the 15 completed runs are in `results/d40_checkpoints/` (local only, gitignored).
...
- **Key findings:**
  ...
  3. **UESD AS BUILT IS A FINITE-TIME TRANSIENT SOLVER, NOT A FIXED-POINT SOLVER.** Correct output lives in a compute window around the trained VT range [4-16]: 99.90-100% per run at T=10 (mean 99.99%), 99.83% mean at T=25, collapse thereafter. The λ=0 CE-only ablation degrades gracefully (52.7% mean at T=200); any λ>=0.5 collapses to ~0%.
  4. **SC SHARPENS THE TRANSIENT AND POISONS THE ENDPOINT.** Stronger SC lowers residual monotonically in per-λ group means (confirming D39; per-seed exception: seed 256 goes 0.083 at λ=0.5 → 0.094 at λ=1.0) and simultaneously accelerates long-horizon accuracy collapse. Self-consistency pressure as implemented stabilizes non-decoding fixed points near — but not at — the correct readout region.
  5. **k DOES RESPOND TO STRONG SC (D39 REVISION).** λ=3.0 spans k=0.940-0.966 (mean 0.951; seeds 42/137 reach 0.940/0.947, seed 256 stays at 0.966) vs D39's "architecture-pinned ~0.96" (measured only up to λ=1.0). But lower k is not the goal: it just speeds convergence to the wrong attractors (finding 2).
