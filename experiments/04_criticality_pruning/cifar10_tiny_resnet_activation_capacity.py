...
RESERVE = 0.60
METHODS = ["magnitude", "global_synflow", "reserve_0.60", "activation_reserve_0.60"]


def module_weight_names(model: nn.Module) -> dict[nn.Module, str]:
    out = {}
    for name, module in model.named_modules():
        if isinstance(module, (nn.Conv2d, nn.Linear)):
            out[module] = f"{name}.weight" if name else "weight"
    return out


def input_activity(model: nn.Module, loader) -> dict[str, torch.Tensor]:
    names = module_weight_names(model)
    ...
    def make_hook(module: nn.Module):
        ...
        def hook(_module, inputs):
            x = inputs[0].detach()
            if x.ndim == 4:
                value = x.abs().mean(dim=(0, 2, 3)).cpu()
            elif x.ndim == 2:
                value = x.abs().mean(dim=0).cpu()
            else:
                value = x.abs().reshape(x.shape[0], -1).mean(dim=0).cpu()
    ...
    for module in names:
        hooks.append(module.register_forward_pre_hook(make_hook(module)))

    model.eval()
    with torch.no_grad():
        for idx, (x, _y) in enumerate(loader):
            if idx >= CALIBRATION_BATCHES:
                break
            model(x.to(base.DEVICE))
...
def activation_supported_scores(model: nn.Module, loader, synflow: dict[str, torch.Tensor], magnitude: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    activity = input_activity(model, loader)
    scores = {}
    for name, score in synflow.items():
        act = activity.get(name)
        if act is None:
            scores[name] = score.detach().clone()
            continue
        act = act.to(score.device)
        act = act / (act.mean().clamp_min(1e-8))
        if score.ndim == 4:
            shaped = act.view(1, -1, 1, 1)
        elif score.ndim == 2:
            shaped = act.view(1, -1)
        else:
            shaped = act
        # Use-dependent capacity rank: keep routes with both saliency and active presynaptic support.
        # Magnitude stabilizes dense/output ranks where SynFlow is known to starve classifier bridges.
        bridge_rank = magnitude[name] if name.endswith("fc.weight") or name == "fc.weight" else score
        scores[name] = (bridge_rank.detach() * shaped).abs().clone()
    return scores


def capacity_mask_with_rank(synflow, magnitude, rank_scores, sparsity: float, reserve: float):
    ...
    weights = {k: base.output_units(synflow[k]) for k in critical}
    ...
    for k in critical:
        ...
        masks[k] = torch.maximum(masks[k], base.topk(rank_scores[k], budget))

    for k in critical:
        layer_mask = masks[k].clone()
        flat_outputs = layer_mask.reshape(layer_mask.shape[0], -1)
        flat_rank = rank_scores[k].reshape(rank_scores[k].shape[0], -1)
    ...
    if remaining > 0:
        ...
        for k, score in synflow.items():
            ...
            if idx.numel() > 0:
                vals = rank_scores[k].flatten()[idx]
                chunks.append((k, idx, vals))
                values.append(vals)
        flat_values = torch.cat(values)
...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_tiny_resnet_activation_capacity.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_TINY_RESNET_ACTIVATION_CAPACITY.md"
    lines = [
        ...
        f"Calibration batches: `{result['calibration_batches']}`",
        "",
...
def run():
    ...
    for seed in base.SEEDS:
        ...
        act_rank = activation_supported_scores(model, train_loader, syn, mag)
        for sparsity in base.SPARSITIES:
            masks_by_label = {
                ...
                "activation_reserve_0.60": capacity_mask_with_rank(syn, mag, act_rank, sparsity, RESERVE),
            }
    ...
    result = {
        "experiment": "04_cifar10_tiny_resnet_activation_capacity",
        "setup": "CIFAR-10 TinyResNet test of activation-supported path-capacity reserve pruning under residual connections.",
        "device": base.DEVICE,
