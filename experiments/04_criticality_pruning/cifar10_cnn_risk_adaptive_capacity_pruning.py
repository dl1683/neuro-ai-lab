...
SEEDS = [161, 162, 163, 164]
SPARSITIES = [0.98, 0.99]
METHODS = ["magnitude", "global_synflow", "fixed_capacity", "risk_adaptive_capacity"]
...
def capacity_mask(synflow, magnitude, sparsity, mode: str):
    ...
    for name in CRITICAL_LAYERS:
        report = reports[name]
        if report.output_units:
            risks[name] = (report.dead_output_units or 0) / report.output_units
        else:
            risks[name] = 0.0
    mean_risk = float(np.mean(list(risks.values())))
    if mode == "fixed_capacity":
        reserve_fraction = 0.55
        weights = {name: float(reports[name].output_units or 1) for name in CRITICAL_LAYERS}
    elif mode == "risk_adaptive_capacity":
        reserve_fraction = min(0.70, max(0.30, 0.30 + 0.40 * mean_risk))
        weights = {name: float(reports[name].output_units or 1) * (0.25 + risks[name]) for name in CRITICAL_LAYERS}
    else:
    ...
    layer_budgets = {}
    for name in CRITICAL_LAYERS:
        output_units = reports[name].output_units or 1
    ...
    meta = {"reserve_fraction": reserve_fraction, "mean_risk": mean_risk, "risks": risks, "layer_budgets": layer_budgets}
    return masks, meta
...
def run():
    ...
    for sparsity in SPARSITIES:
        ...
        for method in METHODS:
            ...
            summary[str(sparsity)][method] = {
                ...
                "reserve_fraction_mean": float(np.mean([r["meta"].get("reserve_fraction", 0.0) for r in selected])) if method in {"fixed_capacity", "risk_adaptive_capacity"} else None,
        ...
        for method in ["global_synflow", "fixed_capacity", "risk_adaptive_capacity"]:
            deltas = []
            for seed in SEEDS:
    ...
    result = {"experiment": "04_cifar10_cnn_risk_adaptive_capacity_pruning", "setup": "CIFAR-10 CNN risk-adaptive path-capacity pruning. Reserve fraction and per-cut allocation are computed from predicted global SynFlow dead-output risk rather than fixed by sparsity.", "critical_layers": CRITICAL_LAYERS, "device": base.DEVICE, "gpu_name": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None, "seeds": SEEDS, "sparsities": SPARSITIES, "methods": METHODS, "summary": summary, "paired_deltas": paired, "rows": rows}
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_cnn_risk_adaptive_capacity_pruning.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_CNN_RISK_ADAPTIVE_CAPACITY_PRUNING.md"
    lines = ["# CIFAR-10 CNN Risk-Adaptive Capacity Pruning", "", result["setup"], "", f"Device: `{result['device']}` / `{result['gpu_name']}`", "", "| Sparsity | Method | Reserve | Before FT | After FT | fc1 keep | Dead fc1 | conv2/conv3/fc2 keep |", "|---:|---|---:|---:|---:|---:|---:|---|"]
    for sparsity in SPARSITIES:
        for method in METHODS:
