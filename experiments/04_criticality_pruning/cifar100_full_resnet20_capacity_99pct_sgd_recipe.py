...
def full_loaders(seed: int):
    ...
    train_ds = datasets.CIFAR100(base.DATA, train=True, download=True, transform=train_tf)
    test_ds = datasets.CIFAR100(base.DATA, train=False, download=True, transform=eval_tf)
    generator = torch.Generator().manual_seed(seed)
    train_loader = DataLoader(train_ds, batch_size=256, shuffle=True, num_workers=2, generator=generator)
...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar100_full_resnet20_capacity_99pct_sgd_recipe.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR100_FULL_RESNET20_CAPACITY_99PCT_SGD_RECIPE.md"
    lines = [
    ...
    for method in METHODS:
        ...
        dense = float(np.mean([row["dense_accuracy"] for row in result["rows"] if row["method"] == method]))
        delta = ""
        wins = ""
        if method != "magnitude":
        ...
        lines.append(
            f"| `{method}` | `{dense:.4f}` | `{item['after_mean']:.4f}` | `{item['after_std']:.4f}` | {delta} | {wins} | "
...
def run():
    rows = []
    for seed in SEEDS:
        print(f"seed {seed}: train dense full CIFAR-100 SGD/cosine", flush=True)
    ...
    result = {
        "experiment": "04_cifar100_full_resnet20_capacity_99pct_sgd_recipe",
        "setup": "Full CIFAR-100 train/test ResNet-20-style path-capacity pruning at 99% sparsity with 20 dense SGD/cosine epochs and 5 masked fine-tune epochs.",
        "device": base.DEVICE,
