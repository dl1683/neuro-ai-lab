...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_resnet20_capacity_99pct_replicate.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_RESNET20_CAPACITY_99PCT_REPLICATE.md"
    lines = ["# CIFAR-10 ResNet-20 Capacity 99% Replicate", "", result["setup"], "", f"Device: `{result['device']}` / `{result['gpu_name']}`", f"Seeds: `{result['seeds']}`", "", "| Method | After FT | After std | Delta vs magnitude | Wins | Main min | Projection min | FC score | Dead outputs |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for method in METHODS:
    ...
    lines.extend(["", "## Interpretation", "", "This is a fresh four-seed replicate of the ResNet-20-style path-capacity transfer at 99% sparsity."])
    md.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run():
    ...
    result = {"experiment": "04_cifar10_resnet20_capacity_99pct_replicate", "setup": "Fresh four-seed replicate for CIFAR-10 ResNet-20-style path-capacity pruning at 99% sparsity.", "device": base.DEVICE, "gpu_name": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None, "seeds": SEEDS, "sparsity": SPARSITY, "reserve": RESERVE, "summary": summary, "paired_deltas": paired, "rows": rows}
    write_report(result)
    return result
