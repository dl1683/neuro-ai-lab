...
def write_report(result):
    result["experiment"] = "04_tinyimagenet_resnet18_pretrained_feature_viability_99pct"
    result["setup"] = "TinyImageNet-200 pretrained ResNet-18 99% sparsity test for magnitude-first feature-subspace preservation plus minimal liveness repair."
    out = pre.ROOT / "results" / "04_criticality_pruning" / "tinyimagenet_resnet18_pretrained_feature_viability_99pct.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = pre.ROOT / "experiments" / "04_criticality_pruning" / "TINYIMAGENET_RESNET18_PRETRAINED_FEATURE_VIABILITY_99PCT.md"
    lines = [
