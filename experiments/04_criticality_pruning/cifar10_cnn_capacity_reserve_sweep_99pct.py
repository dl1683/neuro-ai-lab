...
SPARSITY = 0.99
RESERVES = [0.40, 0.45, 0.50, 0.55, 0.60, 0.65]
...
def fixed_capacity_mask(synflow, magnitude, sparsity, reserve_fraction):
    ...
    weights = {"conv2": 64, "conv3": 64, "fc1": 192, "fc2": 10}
...
def run():
    ...
    for seed in SEEDS:
        ...
        masks_by_label = {
            "magnitude": global_topk_mask(mag, SPARSITY),
            "global_synflow": global_topk_mask(syn, SPARSITY),
        }
        for reserve in RESERVES:
            masks_by_label[f"reserve_{reserve:.2f}"] = fixed_capacity_mask(syn, mag, SPARSITY, reserve)
        for label, masks in masks_by_label.items():
            ...
            rows.append({"seed": seed, "method": label, "reserve": None if not label.startswith("reserve_") else float(label.split("_")[1]), "dense_accuracy": dense_accuracy, "before_accuracy": before, "after_accuracy": after, "before_retention": before / dense_accuracy, "after_retention": after / dense_accuracy, **stats, "damage": layer_damage(masks)})
            print(f"seed {seed} {label}: before={before:.4f} after={after:.4f} dead_fc1={stats['dead_fc1_hidden']}", flush=True)
    ...
    for method in methods:
        ...
        if method != "magnitude":
            ...
            paired.append({"method": method, "before_delta_mean": float(np.mean([d["before_delta"] for d in deltas])), "after_delta_mean": float(np.mean([d["after_delta"] for d in deltas])), "before_wins": int(sum(d["before_delta"] > 0 for d in deltas)), "after_wins": int(sum(d["after_delta"] > 0 for d in deltas)), "paired_rows": deltas})
    best = max([m for m in methods if m.startswith("reserve_")], key=lambda m: summary[m]["after_mean"])
    result = {"experiment": "04_cifar10_cnn_capacity_reserve_sweep_99pct", "setup": "CIFAR-10 CNN 99% path-capacity reserve sweep. Tests whether the fixed reserve fraction has a stable optimum under the same parameter budget.", "device": base.DEVICE, "gpu_name": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None, "seeds": SEEDS, "sparsity": SPARSITY, "reserves": RESERVES, "summary": summary, "paired_deltas": paired, "best_reserve_method": best, "rows": rows}
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_cnn_capacity_reserve_sweep_99pct.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_CNN_CAPACITY_RESERVE_SWEEP_99PCT.md"
    lines = ["# CIFAR-10 CNN Capacity Reserve Sweep at 99%", "", result["setup"], "", f"Device: `{result['device']}` / `{result['gpu_name']}`", "", "| Method | Before FT | After FT | After std | Dead fc1 | fc1 keep | conv2/conv3/fc2 keep |", "|---|---:|---:|---:|---:|---:|---|"]
    for method in methods:
        item = summary[method]
        rates = f"{item['conv2_keep_rate_mean']:.3f}/{item['conv3_keep_rate_mean']:.3f}/{item['fc2_keep_rate_mean']:.3f}"
        lines.append(f"| `{method}` | `{item['before_mean']:.4f}` | `{item['after_mean']:.4f}` | `{item['after_std']:.4f}` | `{item['dead_fc1_hidden_mean']:.1f}` | `{item['fc1_keep_rate_mean']:.4f}` | `{rates}` |")
    ...
    lines.extend(["", f"Best reserve method by after-FT mean: `{best}`."])
    md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return result
...
if __name__ == "__main__":
    result = run()
    print(json.dumps({"best": result["best_reserve_method"], "summary": result["summary"], "paired_deltas": result["paired_deltas"]}, indent=2))
