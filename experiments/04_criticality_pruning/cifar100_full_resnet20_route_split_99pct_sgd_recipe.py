...
SPLITS = {
    "plain_reserve": None,
    "balanced_40_35_25": RouteFamilySplit(main=0.40, projection=0.35, readout=0.25),
    "readout_heavy_35_20_45": RouteFamilySplit(main=0.35, projection=0.20, readout=0.45),
    "readout_main_45_15_40": RouteFamilySplit(main=0.45, projection=0.15, readout=0.40),
}
METHODS = ["magnitude", "plain_reserve", "balanced_40_35_25", "readout_heavy_35_20_45", "readout_main_45_15_40"]
...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar100_full_resnet20_route_split_99pct_sgd_recipe.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR100_FULL_RESNET20_ROUTE_SPLIT_99PCT_SGD_RECIPE.md"
    lines = [
...
def run():
    rows = []
    for seed in SEEDS:
        print(f"seed {seed}: train dense full CIFAR-100 route split", flush=True)
        ...
        masks_by_label = {"magnitude": base.global_mask(mag, SPARSITY)}
        for label, split in SPLITS.items():
            if split is None:
                masks_by_label[label] = base.capacity_mask(syn, mag, SPARSITY, RESERVE)
            else:
    ...
    result = {
        "experiment": "04_cifar100_full_resnet20_route_split_99pct_sgd_recipe",
        "setup": "Full CIFAR-100 train/test ResNet-20-style route-family split test at 99% sparsity using the 20 dense SGD/cosine plus 5 masked fine-tune recipe.",
        "device": base.DEVICE,
        "gpu_name": c100.torch.cuda.get_device_name(0) if c100.torch.cuda.is_available() else None,
        "seeds": SEEDS,
        "sparsity": SPARSITY,
        "reserve": RESERVE,
        "splits": {name: None if split is None else split.__dict__ for name, split in SPLITS.items()},
        "summary": summary,
