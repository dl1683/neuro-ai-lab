...
METHODS = ["magnitude", "reserve_0.60", "tuned_40_35_25", "diversity_target_optimizer"]
...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_tiny_resnet_diversity_route_optimizer_99pct.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_TINY_RESNET_DIVERSITY_ROUTE_OPTIMIZER_99PCT.md"
    lines = [
...
def run():
    ...
    for seed in SEEDS:
        ...
        masks_by_label = {
            ...
            "diversity_target_optimizer": route_split_capacity_mask(syn, mag, SPARSITY, RESERVE, best["split"]),
        }
    ...
    result = {"experiment": "04_cifar10_tiny_resnet_diversity_route_optimizer_99pct", "setup": "Fresh four-seed TinyResNet 99% diversity-penalized target-matched route optimizer.", "device": base.DEVICE, "gpu_name": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None, "seeds": SEEDS, "sparsity": SPARSITY, "reserve": RESERVE, "chosen_splits": chosen, "summary": summary, "paired_deltas": paired, "rows": rows}
    write_report(result)
    return result
