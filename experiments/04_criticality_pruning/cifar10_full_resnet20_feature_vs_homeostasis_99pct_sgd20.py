...
from shared.circuit_viability_selector import choose_ecology_aware_method, choose_unified_viability_family, split_dict
...
SPARSITY = 0.99
RESERVE = 0.60
METHODS = ["magnitude", "feature_viability_repair", "plain_reserve", "predicted_route_split", "unified_policy"]
...
def write_report(result):
    out = ROOT / "results" / "04_criticality_pruning" / "cifar10_full_resnet20_feature_vs_homeostasis_99pct_sgd20.json"
    out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    md = ROOT / "experiments" / "04_criticality_pruning" / "CIFAR10_FULL_RESNET20_FEATURE_VS_HOMEOSTASIS_99PCT_SGD20.md"
    lines = [
    ...
    for item in result["decisions"]:
        lines.append(
            f"- seed `{item['seed']}`: family `{item['selected_family']}` route_floor `{item['magnitude_route_floor']:.4f}` "
            f"method `{item['selected_method']}`"
        )
    lines.extend(
        [
            "",
...
def run():
    ...
    decisions = []
    for seed in SEEDS:
        print(f"seed {seed}: train dense full CIFAR-10 feature-vs-homeostasis", flush=True)
        ...
        print(f"seed {seed}: dense_accuracy={dense_accuracy:.4f}", flush=True)
        mag = base.magnitude_scores(model)
        syn = base.synflow_scores(model)
        mag_mask = base.global_mask(mag, SPARSITY)
        mag_quality = c10.r20.route_quality(mag_mask)
        family = choose_unified_viability_family(magnitude_quality=mag_quality)
        ecology = choose_ecology_aware_method(syn, mag, SPARSITY, RESERVE, base.capacity_mask, base.global_mask, c10.r20.route_quality)
        ...
        if family["selected_family"] == "feature_viability_repair":
            source_method = "feature_viability_repair"
        else:
            source_method = ecology["selected_method"]
        decisions.append(
            {
                "seed": seed,
        ...
        print(f"seed {seed}: family={family['selected_family']} route_floor={family['magnitude_route_floor']:.4f} method={source_method}", flush=True)
        evaluated = {}
        ...
        policy_row["method"] = "unified_policy"
        policy_row["policy_source_method"] = source_method
        rows.append(policy_row)
    summary, paired = summarize(rows)
    result = {
        "experiment": "04_cifar10_full_resnet20_feature_vs_homeostasis_99pct_sgd20",
        "setup": "Fresh full-CIFAR ResNet-20 99% sparsity test comparing magnitude-first feature repair against homeostatic/ecology repair when magnitude has a dead route floor.",
        "device": base.DEVICE,
