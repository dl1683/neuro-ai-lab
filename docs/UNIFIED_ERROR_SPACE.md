...
The canonical-mode SVAMP initial-parser attempt produced 66/256 correct, 177/256 valid extracted incorrect, and 13/256 extraction failures (25.78125% exact-answer accuracy; 5.078125% extraction failures). Although the correct count is inside the 26–217 band and both outcome populations exceed 40, the extraction-failure rate is one example above the preregistered 5% ceiling. The runner therefore recorded **PARSER-REPAIR-REQUIRED**, wrote the immutable initial-miss artifact, and did not write the canonical result path. All 13 failed responses are the literal string `Answer:` with no numeric content; no parser repair or second canonical attempt is claimed here. The task-band gate remains incomplete, mechanics work remains blocked, and this band-placement result supplies no capability evidence or semantic-ratchet evidence.
