...
A successor `base-B` task-band gate is **PREREGISTERED / NOT YET RUN** on a deterministic disjoint GSM8K cohort. It is an independent prerequisite and does not reinterpret, pool, or rescue either base-A result. The 30M mechanics pilot is now formally independent of both task-band gates and may proceed only after its own independent pre-training code review. Full-program launch remains blocked unless the mechanics pilot has an admissible outcome, the base-B task-band gate passes, and a new untouched cross-family transfer set is frozen by pre-data amendment.
